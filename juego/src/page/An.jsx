...
import { Boton_inicio } from "../components/Boton_inicio";
...
export const An = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      console.log("Datos enviados:", { usuario, contraseña: password });

      const response = await axios.post('http://localhost/juego_personajes/backend/login.php', {
        usuario,
        contraseña: password
      }, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setMensajeError(response.data.message || 'Credenciales incorrectas.');
      }
    } catch (error) {
    ...
    <>
      <div className="pricipal flex flex-col items-center justify-center">
        <div className=" max-w-xl w-full p-8 rounded-lg">
          ...
          <form className="shadow-custom-light h-96 xl:h-2/3 w-full rounded-2xl bg-red-300" onSubmit={handleLogin}>
          <Boton_inicio/>
            <div className="flex flex-col gap-3 mx-6 text-center">
              ...
              {nombreUsuario && <p className="text-blue-500 text-center">Bienvenido, {nombreUsuario}!</p>} {/* Mensaje personalizado */}
              <div className="flex flex-col justify-center gap-4">
                <Link to="#" className="text-blue-600 text-xs">Olvidé mi contraseña</Link>
